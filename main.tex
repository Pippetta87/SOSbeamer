...
\begin{figure}[!ht]
\includegraphics[keepaspectratio,height=0.75\textheight]{modes}
\end{figure}
...
In questa tesina mi sono occupato dei modi normali gravo-acustici solari, cio\'e oscillazioni attorno alla struttura di equilibrio le cui caratteristiche sono determinate dal profilo radiale della densit\'a e velocit\'a del suono nell'interno solare.
Le prime osservazioni di un comportamento periodico nell'atmosfera solare sono state effettuate da Leighton all'inizio degli anni '60 e in seguito il fenomeno \'e tstato spiegato come sovrapposizione di modi acustici confinati in cavit'a risonanti. L'eliosismologia \'e la disciplina che studia la propagazione delle onde all'interno del Sole.
Il gran numero di frequenze osservate impone vincoli stringenti alle caratteristiche del modello solare standard.


%Cos'\'e un modo??Sovrapposizione di onde di stessa ampiezza propagantesi in direzioni opposte
...
La legge di conservazione dell'ìenergia interna determina la relazione tra il calore trasferito dalle reazioni nucleari per unit\'a di massa, e il flusso: si ha situazione staziopnaria quando l'energia prodotta dalle reazioni nucleari bilancia il flusso uscente (divergenza del flusso uscente proporzionale a $\nabla T\propto\nabla P$).
...
Nelle condizioni tipiche degli interni stellari i fotoni hanno cammino libero medio molto breve $\invers{(\kappa_{\nu}\rho)}$ quindi posso considerare materia radiazione in LTE: il flusso \'e trasportato dall'anisotropia nella radiazione di Plank dovuta al gradiente termico.
...
Per determinare il gradiente termico ambientale in queste zone si usa la teoria della mixing lenght che introduce il parametro $\alpha$ per descrivere l'efficienza della convezione che fornisce il metodo per ricavare il gradiente ambientale.
...
In questa sezione descrivo il sistema di equazioni che descrive i modi normali adiabatici, le caratteristiche dei modi per alte e basse frequenza, ricavo usando l'approssimazione di Cowling, la relazione di dispersione per onde gravo acustiche e mostro la condizione di risonanza radiale per modi p e modi g.
...
Considero un campo di perturbazioni infinitesi della posizione di equilibrio $\vec{\xi}$ di un elemento di gas: la forza agente sull'elemento \'e determinata da pressione densit\'a e accelerazione di gravit\'a a $\vec{r}+\vec{\xi}$. Ottengo l'equazione del moto perturbata al prim'ordine nelle perturbazioni consioderando il valore attuale di P, $\rho'$, $\vec{g}$ come sovrapposizione della perturbazione euleriana al valore di equilibrio: le quantit\'a primate sono perturbazioni euleriane e $\vec{g}'=-\nabla\Phi'$ e $\nabla^2\Phi'=4\pi G\rho'$.
...
\captionof{figure}{Da \cite{christensen1985speed}.}
\end{figure}
...
\begin{equation*}
\sum_ic_i(r_0)\frac{\delta\omega_i}{\omega_i}=\int_0^R\sum_ic_i(r_0)K^i(r)\frac{\delta f(r)}{f(r)}\,dr=\int_0^R\mathcal{K}(r_0,r)\frac{\delta f(r)}{f(r)}\,dr=\exv{\frac{\delta f(r_0)}{f(r_0)}}
\end{equation*}
