...
\begin{block}{Schermaggio debole}

L'equazione di Poisson per $\phi$:
\begin{equation}\label{eq:poissonscreened}
\nabla^2\phi=-4\pi e\sum Zn_Z-4\pi\sum Z_i\delta(\vec{r}-\vec{r}_i)
\end{equation}
...
\end{equation}
quindi ho l'equazione di Poisson \eqref{eq:poissonscreened} a distanza $|\vec{r}-\vec{r}_i|$ dallo ione $Z_i$ in $\vec{r}_i$:
\begin{equation}\label{eq:poissonspheric}
\frac{r_D^2}{r_i}\TtwoDy{r}{(r_i\phi_i)}
\end{equation}
con
\begin{equation}
\phi=\sum_i\phi_i
