...
\begin{frame}<presentation:0>[noframenumbering]{Correzioni alla legge dei gas perfetti: Interazioni coulombiane}
...
\begin{equation*}
\frac{1}{2}\TtwoDy{t}{I}=2E_i+\Omega\approx0%\approx\int_M\frac{3P}{\rho}\,dm(r)+\Omega
\end{equation*}

Detta $W=E_i+\Omega$ l'energia totale della stella, si ha: $\Omega=-2E_i$.

%Cammino libero medio degli atomi e dei fotoni breve: l'equilibrio idrostatico e termico locale.
%\begin{equation}
...
\begin{frame}<presentation:0>[noframenumbering]{Urti}
...
\begin{frame}{Trasferimento impulso nelle collisioni}
...
La diffusione termica concentra le particelle pi\'u pesanti (e pi\'u cariche) nelle zone pi\'u calde: in presenza di un gradiente termico si ha un trasferimento netto di momento negli urti in direzione del gradiente, in \eqref{eq:resistance}%( dipendenz della sezione d'urto coulombiana dalla velocit\'a relativa $\propto v\expy{-4}$/probabilit\'a di collisione $\propto v\expy{-3}$)).
...
\begin{frame}<presentation:0>[noframenumbering]{Contributi a $\kappa$ per processoper elemento}
...
\subsection{Condizione di in-stabilit\'a dinamica: teoria della mixing-length}
...
\begin{frame}{Flusso convettivo}

\begin{figure}[!h]
    \includegraphics[width=0.7\textwidth,keepaspectratio]{proportionflux}
...
\end{equation}
%dove $Q_{ij}$ \'e l'energia liberata per reazione tra nucleo di specie i e j
...
\node[anchor=north ]  at (4,-5.5) {\parbox{0.99\textwidth}{\captionof{figure}{Reazioni catena PP: terminazioni per temperature caratteristiche del centro solare. Da \cite{adelberger2011solar}.}}};
...
\node (pep) at ($(s.south)+(0mm,-4mm)$) {\parbox{\textwidth}{$\scriptscriptstyle II: R(p\Pelectron p)=\scriptscriptstyle1.102(1\pm0.01)10^{-4}\midfrac{\rho}{\mu_e}T_6\expy{-1/2}
\scriptscriptstyle[1+0.002(T_6-16)]R(pp)$}};
\node[below=0.1mm of pep] (ec) {\parbox{\textwidth}{$\scriptscriptstyle I: R(\cel{Be}{7}{}{}(\Pelectron,\Pnue)\cel{Li}{7}{}{})=\scriptscriptstyle5.6(1\pm0.02)10^{-9}\midfrac{\rho}{\mu_e}T_6\expy{-1/2}\scriptscriptstyle[1+0.004(T_6-16)]\si{\per\second}$}};
 
\node[below=0.1mm of ec] (captions) {\parbox{\textwidth}{\captionof{figure}{Fattore astrofisico reazioni catena PP. Da \cite{adelberger2011solar}.}}};
...
\begin{column}{0.6\textwidth}
\begin{figure}[!ht]
       \includegraphics[width=0.8\textwidth,keepaspectratio]{Rscreening}
\end{figure}
\end{column}

\begin{column}{0.4\textwidth}
\captionof{figure}{Andamento della correzione di schermaggio degli elettroni secondo diversi schemi. Da \cite{ricci1995screening}.}
\end{column}
\end{columns}
...
\begin{figure}[!h]
\includegraphics[width=0.75\textwidth,trim=4 4 4 4,clip]{BP00-SSM-M}
...
\begin{figure}[!h]
\includegraphics[width=0.75\textwidth,trim=4 4 4 4,clip]{BP00-cs}
\caption{Profilo radiale della densit\'a, abbondanza di idrogeno, luminosit\'a e temperatura; sono indicati i valori centrali e alla base della zona convettiva. Dati da \cite{BP2000}.}
\end{figure}
...
\begin{column}{0.5\textwidth}
\begin{figure}[!ht]
\includegraphics[width=0.95\textwidth,keepaspectratio]{dsoundspeedduvall} 
\end{figure}
\end{column}
\begin{column}{0.5\textwidth}
\captionof{figure}{Differenza relativa del profilo di $c_s$ (determinata invertendo \eqref{eq:analinversionc}) per frequenze dei modi calcolate con MSS e osservate. La differenza relativa \'e minore del $5\%$. Da \cite{christensen1985speed}.}
\label{dsoundduvall}
\end{column}
\end{columns}
...
%Il gradiente adimensionale della velocit\'a del suono $W$ mette in risalto la discontinuit\'a del gradiente termico (alla base della zona convettiva) e in $\Gamma_1$ (nelle zone di ionizzazione):
\begin{equation}
