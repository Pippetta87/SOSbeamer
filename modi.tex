...
Rumore solare a basse frequenze e ai processi di eccitazione dei modi che trasferiscono la massima energia per $\Pi\approx\SI{5}{\minute}$.
...
&P(\vec{r},t)=P_0(\vec{r})+P'(\vec{r},t)\label{eq:pressureperturbation}\\
&\Lvar{P(\vec{r})}=P(\vec{r}+\Lvar{\vec{r}})-P_0(\vec{r})=P'(\vec{r})+\xi\cdot\nabla P_0\\
&\vec{g}'=-\nabla\Phi',\ \nabla^2\Phi'=4\pi G\rho'\label{eq:gapert}
\end{align}
...
\begin{figure}[!ht]
\subfigure[I picchi della densit\'a spettrale si dispongono su creste nel diagramma $\nu-l$.]{\includegraphics[keepaspectratio,width=0.4\textwidth]{midlmodes}}
\label{fig:midlmodes}
~
\subfigure[Modi adiabatici calcolati sulla base di un modello solare. .]{\includegraphics[keepaspectratio,width=0.4\textwidth]{nrmodesLAWE}}
...
\begin{frame}<presentation:0>[noframenumbering]{Accordo frequenze osservate vs calcolate tramite MSS}
...
%\subfigure[Differenza di fase per il segnale Doppler delle righe $(5930)$ Fe I, pi\'u profonda, e $(5896)$ Na I, pi\'u alta nell'atmosfera. Da \cite{staiger1987observations}.]{\includegraphics[keepaspectratio,width=0.4\textwidth]{phasepropagation}}
%\label{fig:phasedifference}
%~
%\subfigure[In alto: differenze tra frequenze dei modi teoriche e osservate. In basso: differenze moltiplicata per $Q_{nl}$.]{\includegraphics[keepaspectratio,width=0.4\textwidth]{domega}}
%\label{fig:nFreqdiff}
\end{figure}
\end{column}
...
\begin{figure}[!ht]
\centering
\subfigure[Densit\'a spettrale modi p di basso grado angolare. Da \cite{chr02helioseismology}.]{\includegraphics[keepaspectratio,width=0.5\textwidth]{lowlmodes}}
%\caption{Densit\'a spettrale modi p di basso grado angolare. Da \cite{chr02helioseismology}.}\label{fig:lowlmodes}
%\begin{figure}[!ht]
%\centering
\subfigure[Densit\'a spettrale di oscillatore smorzato di frequenza naturale $\midfrac{\omega_{nl}}{2\pi}$ con forzante stocastica. $P_f$ \'e la DSP della forzante. $P=P_fP_L$. Da \cite{houdek2006stochastic}.]{\includegraphics[keepaspectratio,width=0.35\textwidth]{Powerspectraldensity}}
...
\cos{(m\phi'-\omega_{n,l}t)}=\cos{(m\phi-\omega_{n,l,m}t)}\\
\Delta\omega_{(l,m)}=m\Omega
\end{align*}
...
\captionof{figure}{Diagramma $\omega-k_h$ per osservazione di una regione solare $\ang{;;2}\times\ang{;;2}$ mediata in direzione nord-sud. Si osserva lo spostamento dovuto alla rotazione dalla predizione per modello non-rotante (linee continue). Da \cite{rhodes1979new}.}\label{fig:rotationshiftridge}

I modi di oscillazioni sono onde stazionarie nel rifermento corotante mentre nel riferimento inerziale si ha uno spostamento delle frequenze per le onde che hanno una componente parallela all'equatore.
...
\begin{figure}[!ht]
\centering\includegraphics[keepaspectratio,angle=0,width=0.75\textwidth]{rotationshiftridge}
\end{figure}
...
\begin{frame}<presentation:0>[noframenumbering]{Splitting delle frequenze per campo velocit\'a rotazionale}
...
\begin{block}{Relazione di dispersione modi gravo-acustici}
...
\node[inner sep=0pt] (image) at (-0.5,0)
  {\includegraphics[keepaspectratio=true,width=0.6\textwidth]{cutoff}};
  %\node (legenda) at (4.8,2) { Legenda };
  %\draw [draw=black!50] (legenda.north west) rectangle +(0.35\textwidth, -4cm);
  \draw [dotted] (3.5,1.5) -- (4.5,1.5) node[at start, anchor=east] {$\frac{\omega_c}{2\pi}$};
  \draw [dashed] (3.5,0.5) -- (4.5,0.5) node[at start, anchor=east] {$\frac{S_l}{2\pi}$};
  \draw [] (3.5,-1.) -- (4.5,-1.) node[at start, anchor=east] {$\frac{N}{2\pi}$};
...
\begin{block}{Onde stazionaria in direzione radiale}
\begin{equation}
